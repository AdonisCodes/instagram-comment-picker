import { Avatar, Box, Button, Card, CardBody, CardFooter, CardHeader, Flex, Heading, IconButton, Image, Link, Text } from '@chakra-ui/react'
import { useState, useEffect } from 'react';
import axios from 'axios';

export default function WinnerCard({ user }) {
    ...
    return (
        <Card maxW='md' w='250px'>
            <CardHeader>
                <Flex spacing='4'>
                    <Flex flex='1' gap='4' alignItems='center' flexWrap='wrap'>
                        <Avatar name='Segun Adebayo' src={profilePic} />
  
                        <Box>
                            <Link size='sm' href={"https://instagram.com/" + user.ownerUsername + '/'} target='_blank'>{user.ownerUsername}</Link>
                        </Box>
                    </Flex>
                </Flex>
            </CardHeader>
